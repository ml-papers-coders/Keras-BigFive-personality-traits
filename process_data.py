...
def build_data_cv(datafile, clean_string=True):
    ...
    with open(datafile, "r",encoding="utf-8") as csvf:
        csvreader=csv.reader(csvf,delimiter=',',quotechar='"')
        first_line=True
        for line in csvreader:
            # dont process first line
            if first_line:
                first_line=False
                continue
            #print(line[2:])
            #exit()
            #status to save the sentences and splitted long sentences
            status=[]
            #list of sentences
            sentences=re.split(r'[.?]', line[1].strip())
            try:
                #remove an empty sentence from the list
                sentences.remove('')
            except ValueError:
                None
...
def get_W(word_vecs, k=300):
    ...
    word_idx_map = dict()
    W = np.zeros(shape=(vocab_size+1, k), dtype=float)
...
def load_bin_vec(fname, vocab):
    ...
    """
    pretrained=gensim.models.KeyedVectors.load_word2vec_format(fname, binary=True)
    w2v_pretrained={word:pretrained.word_vec(word) for word in vocab if word in pretrained.vocab.keys() }
    del pretrained
    return w2v_pretrained
