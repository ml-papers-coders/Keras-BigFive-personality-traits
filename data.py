...
def load_data(attr,mini_batch_size=50):
    ...
    for i in r[:1]:
        ...
        for t,v,_ in mini_batches_generator:
                ...
                val_set_x=datasets[0][v].reshape((t.shape[0],-1))
                train_set_y=datasets[1][t].reshape((t.shape[0],-1))
                val_set_y=datasets[1][v].reshape((t.shape[0],-1))
                train_set_m=datasets[4][t].reshape((t.shape[0],-1))
                val_set_m=datasets[4][v].reshape((t.shape[0],-1))
                test_set_x = datasets[2].reshape((t.shape[0],-1))
                test_set_y = np.asarray(datasets[3],int).reshape((t.shape[0],-1))
                test_set_m = datasets[5].reshape((t.shape[0],-1))
